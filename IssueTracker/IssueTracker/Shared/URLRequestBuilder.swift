...
struct URLRequestBuilder {
  ...
  static let allHeaderFields = ["Content-Length",
                                 "Authorization",
                                 "Connection",
                                 "Host",
                                 "Proxy-Authenticate",
                                 ...
                                 "WWW-Authenticate",
                                 "Content-Type"]
  static let allHTTPMethods = ["GET", "POST", "PUT", "DELETE", "PATCH"]
  ...
  mutating func setHTTPMethod(_ method: String) {
    ...
    Self.allHTTPMethods.forEach { methodName in
...
//    let index = Self.allHeaderFields.firstIndex(where: { $0.lowercased() == queryString })
    let index = Self.allHeaderFields.firstIndex(where: { $0.lowercased().contains(queryString) })
    
    if let index = index {
      return Self.allHeaderFields[index]
    }
